...
class ArrivalTrials(unittest.TestCase):
    ...
    def test_full_host_arrival_and_return_in_fresh_processes(self):
        ...
        for index in range(5):
            ...
            self.assertFalse(packet["response_constraints"]["serialization"]["embedded_double_quotes_allowed"])
    ...
    def test_response_text_refuses_embedded_double_quotes(self):
        state = self.prepare()
        packet_hash = state["packet_sha256"]
        response = self.answer(packet_hash)
        response["response"]["observations"] = ['quoted "text"']
        with self.assertRaisesRegex(ValueError, "double-quote"):
            arrival.respond(self.output, packet_hash, packet_hash, response)
