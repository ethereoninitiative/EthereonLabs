...
| Resonant Return Panel | `LuminaOS/bootstrap/Ship_of_Ethereon_V2/runtime/resonant_return_panel_r1.py`, `docs/HABITAT_RESONANT_RETURN_GATE_R1.md` | Standalone read-only JSON orientation from explicitly supplied snapshots; synthetic sample plus verified local return evidence through `studio/lumina_bridge_return_r1.py`; advisory only, no default-host runtime wiring |
| Interresident Resonance Protocol R1 | `LuminaOS/bootstrap/Ship_of_Ethereon_V2/runtime/interresident_resonance_r1.py`, V2 `docs/Interresident_Resonance_Protocol_R1.md` | Standalone bounded structured interchange, hash-bound evidence/lineage, disagreement-preserving comparison, and orientation projection; synthetic three-origin demo, no authority or identity authentication |
