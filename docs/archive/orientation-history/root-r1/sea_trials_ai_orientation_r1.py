...
def main():
    ...
    record = protocol.begin(model_provider="test-provider", model_name="test-model", account_label="sea-trial")
    ...
    checks = {
        "all_stages_presented_in_manifest_order": presented == [
            "baseline", "authority", "runtime", "orientation", "transfer_probe"
        ],
        "record_completed": summary["status"] == "completed",
        "all_responses_hashed": len(summary["stage_response_hashes"]) == 5,
        "authority_is_supplemental": summary["authority"].startswith("supplemental orientation only"),
        "claim_boundary_rejects_identity_proof": "does not prove identity transfer" in summary["claim_boundary"],
    }
    report = {"suite": "Sea Trials AI Orientation r1", "passed": all(checks.values()), "checks": checks, "summary": summary}
